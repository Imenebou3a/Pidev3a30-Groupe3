...
public class KitFrontController {
    ...
    @FXML private ScrollPane detailsContent;
    ...
    @FXML private Label detailsPrix;
    @FXML private Label detailsStock;
    @FXML private Label detailsDescription;
    @FXML private Button btnDetailsAjouterPanier;
    @FXML private Button btnDetailsAjouterFavoris;
    ...
    private VBox creerCardKit(KitHobbies kit) {
        ...
        btnFavoris.setStyle("-fx-background-color: #ef4444; -fx-text-fill: white; " +
                           "-fx-font-weight: bold; -fx-font-size: 13px; -fx-background-radius: 8px; " +
                           "-fx-cursor: hand; -fx-padding: 10 16;");
        btnFavoris.setOnAction(e -> ajouterAuxFavoris(kit));
        ...
        buttonsBox.getChildren().addAll(btnFavoris, btnPanier);
        ...
        card.setOnMouseClicked(e -> afficherDetailsDansPanneau(kit));
    ...
    /**
     * Afficher les détails d'un kit dans le panneau de droite
     */
    private void afficherDetailsDansPanneau(KitHobbies kit) {
        kitSelectionne = kit;
        ...
        detailsNom.setText(kit.getNomKit());
        detailsType.setText(kit.getTypeArtisanat());
        
        // Badge niveau avec couleur
        detailsNiveau.setText(kit.getNiveauDifficulte());
        switch (kit.getNiveauDifficulte()) {
            case "Facile":
                detailsNiveau.setStyle("-fx-background-color: #d4edda; -fx-text-fill: #155724; " +
                                      "-fx-padding: 6 12; -fx-background-radius: 15px; -fx-font-size: 12px; -fx-font-weight: bold;");
                break;
            case "Moyen":
                detailsNiveau.setStyle("-fx-background-color: #fff3cd; -fx-text-fill: #856404; " +
                                      "-fx-padding: 6 12; -fx-background-radius: 15px; -fx-font-size: 12px; -fx-font-weight: bold;");
                break;
            case "Difficile":
                detailsNiveau.setStyle("-fx-background-color: #f8d7da; -fx-text-fill: #721c24; " +
                                      "-fx-padding: 6 12; -fx-background-radius: 15px; -fx-font-size: 12px; -fx-font-weight: bold;");
                break;
        }
        
        detailsPrix.setText(String.format("%.2f TND", kit.getPrix()));
        detailsStock.setText(kit.getStock() + " kits");
        detailsStock.setStyle("-fx-font-size: 15px; -fx-font-weight: bold; -fx-text-fill: " + 
                             (kit.getStock() < 5 ? "#ef4444" : "#10b981") + ";");
        
        String description = kit.getDescription();
        if (description == null || description.trim().isEmpty()) {
            description = "Aucune description disponible pour ce kit.";
        }
        ...
        try {
            File imageFile = new File("src/main/resources/images/" + kit.getImageUrl());
            if (imageFile.exists()) {
                detailsImage.setImage(new Image(imageFile.toURI().toString()));
            } else {
                detailsImage.setImage(null);
            }
        } catch (Exception e) {
            detailsImage.setImage(null);
        ...
        btnDetailsAjouterPanier.setDisable(kit.getStock() <= 0);
        btnDetailsAjouterPanier.setOnAction(e -> {
            ajouterAuPanier(kit);
        });
        
        btnDetailsAjouterFavoris.setOnAction(e -> {
            ajouterAuxFavoris(kit);
        });
        
        // Animation d'apparition
        detailsPanel.setStyle(detailsPanel.getStyle() + "-fx-scale-x: 0.95; -fx-scale-y: 0.95;");
        ScaleTransition st = new ScaleTransition(Duration.millis(200), detailsPanel);
        st.setToX(1.0);
        st.setToY(1.0);
        st.play();
    }
    ...
    /**
     * Ajouter un kit aux favoris
     */
    private void ajouterAuxFavoris(KitHobbies kit) {
        try {
            Favori favori = new Favori();
            favori.setIdUtilisateur(idUtilisateur);
            favori.setTypeItem("KIT");
            favori.setIdItem(kit.getIdKit());
            ...
            if (success) {
                ...
                FavorisNotificationService.getInstance().afficherNotificationDejaDans(kit.getNomKit());
            }
        } catch (Exception e) {
