...
public class MainFrontController {
    ...
    @FXML private Label lblBadgeFavoris;
    ...
    private void mettreAJourFavoris() {
        Utilisateur user = SessionManager.getInstance().getUtilisateurConnecte();
        if (user != null && lblBadgeFavoris != null) {
            int nbFavoris = favoriService.compterFavoris(user.getIdUtilisateur());
            
            if (nbFavoris > 0) {
                lblBadgeFavoris.setText(String.valueOf(nbFavoris));
                lblBadgeFavoris.setVisible(true);
            } else {
                lblBadgeFavoris.setVisible(false);
            }
        }
    }
    ...
    public void afficherFavoris() {
        try {
            FXMLLoader loader = new FXMLLoader(
                getClass().getResource("/fxml/frontoffice/favoris.fxml")
            );
            
            if (loader.getLocation() == null) {
                System.err.println("FXML favoris.fxml introuvable!");
                Alert alert = new Alert(Alert.AlertType.ERROR);
                alert.setTitle("Erreur");
                alert.setHeaderText("Fichier FXML introuvable");
                alert.setContentText("Le fichier favoris.fxml n'existe pas dans /fxml/frontoffice/");
                alert.showAndWait();
                return;
            }
            ...
            stage.setOnHidden(e -> mettreAJourFavoris());
            ...
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setTitle("Erreur");
            alert.setHeaderText("Impossible d'ouvrir les favoris");
