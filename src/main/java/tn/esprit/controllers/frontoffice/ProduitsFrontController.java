...
public class ProduitsFrontController {
    ...
    @FXML private Label detailsCategorie;
    @FXML private Label detailsRegion;
    ...
    private VBox creerCardProduit(ProduitLocal produit) {
        ...
        btnFavoris.setOnAction(e -> ajouterAuxFavoris(produit));
        ...
        card.setOnMouseClicked(e -> afficherDetailsDansPanneau(produit));
    ...
    /**
     * Afficher les détails d'un produit dans le panneau de droite
     */
    private void afficherDetailsDansPanneau(ProduitLocal produit) {
        ...
        detailsNom.setText(produit.getNom());
        detailsCategorie.setText(produit.getCategorie());
        detailsRegion.setText("📍 " + produit.getRegion());
        detailsPrix.setText(String.format("%.2f TND", produit.getPrix()));
        detailsStock.setText(produit.getStock() + " unités");
        detailsStock.setStyle("-fx-font-size: 15px; -fx-font-weight: bold; -fx-text-fill: " + 
                             (produit.getStock() < 5 ? "#ef4444" : "#10b981") + ";");
     ...
     */
    private void ajouterAuxFavoris(ProduitLocal produit) {
        try {
            Favori favori = new Favori();
            favori.setIdUtilisateur(idUtilisateur);
            favori.setTypeItem("PRODUIT");
            favori.setIdItem(produit.getIdProduit());
            ...
            if (success) {
                ...
                FavorisNotificationService.getInstance().afficherNotificationDejaDans(produit.getNom());
            }
        } catch (Exception e) {
            showError("Erreur lors de l'ajout aux favoris: " + e.getMessage());
        }
    }
